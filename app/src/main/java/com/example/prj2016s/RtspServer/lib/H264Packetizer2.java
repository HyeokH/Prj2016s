 ...
 *   
 */
public class H264Packetizer2 extends AbstractPacketizer implements Runnable {

	public final static String TAG = "H264Packetizer2";
...
*/
	...
	private void send() throws IOException, InterruptedException {
		...
		byte[] header = new byte[5];
			...
			Log.d(TAG, "header : " + header[0] + " " + header[1] + " " + header[2] + " " + header[3] + " " + header[4]);
			ts += delay;
			naluLength = header[3]&0xFF | (header[2]&0xFF)<<8 | (header[1]&0xFF)<<16 | (header[0]&0xFF)<<24;
			if (naluLength>100000 || naluLength<0) {
				resync();
				Log.d(TAG, "resynced_header : " + header[0] + " " + header[1] + " " + header[2] + " " + header[3] + " " + header[4]);
			}
